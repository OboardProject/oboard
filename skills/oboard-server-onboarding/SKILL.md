...
6. Present the generated install command for the user to run in their own terminal. Issued install commands set `OBOARD_INSTALL_BBR` to `0` or `1`; they must not pass the literal `${OBOARD_INSTALL_BBR:-0}` as the value. The server's `tcp_tuning_enabled` switch is delivered the same way through `OBOARD_INSTALL_TCP_TUNING`. Treat enrollment material as a one-time secret.
