...
6. Present the generated install command for the user to run in their own terminal. Issued install commands set `OBOARD_INSTALL_BBR` to `0` or `1`; they must not pass the literal `${OBOARD_INSTALL_BBR:-0}` as the value. TCP tuning is delivered the same way through `OBOARD_INSTALL_TCP_TUNING`. Both values come from the panel-wide settings `server_default_bbr_enabled` / `server_default_tcp_tuning_enabled`; servers have no per-server BBR or TCP tuning field. Treat enrollment material as a one-time secret.
