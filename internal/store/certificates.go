...
func (s *Store) CreateCertificate(ctx context.Context, v *model.Certificate) error {
	...
	res, err := s.db.ExecContext(ctx, `insert into certificates(name,primary_domain,domains_json,wildcard,challenge_type,dns_credential_id,issuance_server_id,acme_ca,account_email,status,certificate_pem,fullchain_pem,private_key_encrypted,revision,not_before,not_after,auto_renew,validation_records_json,last_error,last_issued_at,last_renewal_attempt_at,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, v.Name, v.PrimaryDomain, string(domains), boolInt(v.Wildcard), v.ChallengeType, v.DNSCredentialID, v.IssuanceServerID, v.ACMECA, v.AccountEmail, v.Status, v.CertificatePEM, v.FullchainPEM, v.PrivateKeyEncrypted, v.Revision, timePtrString(v.NotBefore), timePtrString(v.NotAfter), boolInt(v.AutoRenew), string(validation), v.LastError, timePtrString(v.LastIssuedAt), timePtrString(v.LastRenewalAttemptAt), ts, ts)
	if err != nil {
		return err
	}
	v.ID, _ = res.LastInsertId()
