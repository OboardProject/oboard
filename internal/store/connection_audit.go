...
const connectionAuditRetention = 30 * 24 * time.Hour
...
func (s *Store) AddConnectionAuditReports(ctx context.Context, reports []model.ConnectionAuditReport) ([]string, error) {
	...
	for _, report := range reports {
		...
		res, err := tx.ExecContext(ctx, `insert or ignore into connection_audit_reports(report_id,server_id,user_id,inbound_id,path_id,source_ip,source_geo_code,network,destination,destination_port,outbound_tag,outbound_type,connection_count,active_peak,active_at_end,started_at,ended_at,created_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			report.ReportID, report.ServerID, report.UserID, report.InboundID, report.PathID, report.SourceIP, report.SourceGeoCode, report.Network, report.Destination, report.DestinationPort, report.OutboundTag, report.OutboundType, report.ConnectionCount, report.ActivePeak, report.ActiveAtEnd, report.StartedAt.UTC().Format(time.RFC3339Nano), report.EndedAt.UTC().Format(time.RFC3339Nano), ts)
		if err != nil {
			return nil, err
		}
...
func (s *Store) ConnectionAuditOverview(ctx context.Context, windowHours int) (model.ConnectionAuditOverview, error) {
	...
	overview := model.ConnectionAuditOverview{WindowHours: windowHours, GeneratedAt: nowTime, Users: []model.ConnectionAuditUserSummary{}}
	...
	for index := range overview.Users {
		...
		item.RiskScore, item.RiskLevel, item.RiskSignals = connectionAuditRisk(*item, windowHours)
...
func (s *Store) ConnectionAuditUserDetail(ctx context.Context, userID int64, windowHours int) (model.ConnectionAuditUserDetail, error) {
	...
	detail := model.ConnectionAuditUserDetail{
		...
		Outbounds: []model.ConnectionAuditDimension{}, Servers: []model.ConnectionAuditDimension{}, Recent: []model.ConnectionAuditReport{},
	}
	found := false
	...
	detail.Sources, err = s.connectionAuditDimensions(ctx, `select source_ip,source_ip,coalesce(max(source_geo_code),''),sum(connection_count),max(active_peak),max(ended_at) from connection_audit_reports where user_id=? and ended_at>=? group by source_ip order by sum(connection_count) desc limit 20`, userID, since)
	if err != nil {
		return detail, err
	}
...
func (s *Store) listRecentConnectionAudits(ctx context.Context, userID int64, since string, limit int) ([]model.ConnectionAuditReport, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ReportID, &item.ServerID, &item.UserID, &inboundID, &pathID, &item.SourceIP, &item.SourceGeoCode, &item.Network, &item.Destination, &item.DestinationPort, &item.OutboundTag, &item.OutboundType, &item.ConnectionCount, &item.ActivePeak, &item.ActiveAtEnd, &startedAt, &endedAt, &createdAt); err != nil {
			return nil, err
		}
		if inboundID.Valid {
...
func connectionAuditRisk(item model.ConnectionAuditUserSummary, windowHours int) (int, string, []string) {
	if windowHours < 1 {
		windowHours = 24
	}
	ipMedium, ipHigh := 5, 15
	subnetMedium, subnetHigh := 3, 8
	sharedIPMedium, sharedIPHigh := 2, 5
	serverThreshold := 3
	switch {
	case windowHours > 7*24:
		ipMedium, ipHigh = 25, 60
		subnetMedium, subnetHigh = 10, 25
		sharedIPMedium, sharedIPHigh = 8, 20
		serverThreshold = 5
	case windowHours > 24:
		ipMedium, ipHigh = 12, 30
		subnetMedium, subnetHigh = 6, 15
	...
	if item.SourceIPCount >= ipHigh {
		add(35, "当前窗口内来源 IP 达 "+strconv.Itoa(item.SourceIPCount)+" 个")
	} else if item.SourceIPCount >= ipMedium {
		add(20, "当前窗口内来源 IP 达 "+strconv.Itoa(item.SourceIPCount)+" 个")
	}
	if item.SourceSubnetCount >= subnetHigh {
		add(25, "来源跨 "+strconv.Itoa(item.SourceSubnetCount)+" 个网段")
	} else if item.SourceSubnetCount >= subnetMedium {
		add(15, "来源跨 "+strconv.Itoa(item.SourceSubnetCount)+" 个网段")
	}
