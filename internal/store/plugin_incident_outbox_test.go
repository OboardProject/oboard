...
func TestNodeIncidentOutboxIsIndependentOfNotifyAndDoesNotRefireSameIncident(t *testing.T) {
	...
	s, err := Open(t.TempDir() + "/plugin-incidents.sqlite")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = s.Close() })
	server := &model.Server{Name: "offline-node", AgentID: "agent-plugin-1", Status: model.ServerOffline}
	...
	events, err := s.ClaimPluginEvents(ctx, "test", now.Add(time.Minute), 16)
	if err != nil || len(events) != 1 || events[0].Topic != "plugin.server.offline" {
		t.Fatalf("first offline outbox: %#v err=%v", events, err)
	}
	if err := s.CompletePluginEvent(ctx, events[0].ID); err != nil {
		t.Fatal(err)
	}
	...
	again, err := s.ClaimPluginEvents(ctx, "test", now.Add(time.Minute), 16)
	...
	flapEvents, err := s.ClaimPluginEvents(ctx, "test", now.Add(time.Minute), 16)
