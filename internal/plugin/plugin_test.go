...
func testPluginEnv(t *testing.T) (*store.Store, *Service, *model.User) {
	t.Helper()
	db, err := store.Open(t.TempDir() + "/plugins.sqlite")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = db.Close() })
	user := &model.User{Username: "plugin-admin", PasswordHash: "hash", Role: model.RoleAdmin, Status: "active", ProxyUUID: "plugin-uuid", ProxyPassword: "plugin-pass"}
...
func testManifest(capabilities ...string) json.RawMessage {
	if len(capabilities) == 0 {
		capabilities = []string{model.PluginSDKServersStatus}
	}
	return MustJSON(model.PluginManifest{
		SchemaVersion: model.PluginSchemaVersion,
		Runtime:       model.PluginRuntimeOBoardJSv1,
		SDKVersion:    model.PluginSDKVersionV1,
		Entry:         "main",
...
func TestValidateRevisionDoesNotCompileSource(t *testing.T) {
	_, svc, user := testPluginEnv(t)
	ctx := context.Background()
	actor := adminActor(user)
	plugin, err := svc.CreatePlugin(ctx, actor, "validate-only", "")
	if err != nil {
		t.Fatal(err)
	}
	source := "function main(){ while(true){} }"
	result, err := svc.ValidateRevision(ctx, actor, plugin.ID, source, testManifest(), json.RawMessage(`{}`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseManifestRejectsUnknownFieldsAndReservedEnv(t *testing.T) {
	...
	manifest := model.PluginManifest{
		SchemaVersion: 1, Runtime: model.PluginRuntimeOBoardJSv1, SDKVersion: model.PluginSDKVersionV1,
		Entry: "main", Capabilities: []string{model.PluginSDKServersGet},
		Env: []model.PluginEnvDeclaration{{Name: "OBOARD_RUN_ID"}},
	}
...
func TestScheduleUsesPersistentSlotsAndSkipsDSTGap(t *testing.T) {
	spec, err := ParseTriggerSpec(MustJSON(model.PluginTriggerSpec{Timezone: "America/New_York", Cron: "0 2 * * *"}))
	if err != nil {
		t.Fatal(err)
	}
	...
	event := EventKey(3, 8, 11, model.PluginEventServerOffline)
...
func TestOperatorCannotAuthorizeOrChangeRuntime(t *testing.T) {
	...
	plugin, err := svc.CreatePlugin(ctx, admin, "gated", "")
	if err != nil {
		t.Fatal(err)
	}
	rev, err := svc.SaveDraft(ctx, admin, plugin.ID, "function main(){return {ok:true}}", testManifest(model.PluginSDKHostPoweroff))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.Publish(ctx, admin, plugin.ID, rev.ID); err != nil {
		t.Fatal(err)
	}
	operator := operatorActor(user)
	if _, err := svc.CreateGrant(ctx, operator, model.PluginGrant{
		RevisionID:        rev.ID,
		CapabilitiesJSON:  MustJSON([]string{model.PluginSDKHostPoweroff}),
	...
	if err := svc.UpdateSettings(ctx, operator, Settings{Enabled: true, MaxConcurrency: 2, MaxTimeoutSeconds: 30}); err == nil {
		t.Fatal("operator must not change global plugin settings")
	}
	_ = db
}
...
func TestStalePowerGrantRejectedAfterRevisionChange(t *testing.T) {
	...
	ctx := context.Background()
	actor := adminActor(user)
	plugin, err := svc.CreatePlugin(ctx, actor, "power", "")
	if err != nil {
		t.Fatal(err)
	}
	rev, err := svc.SaveDraft(ctx, actor, plugin.ID, "function main(){return {v:1}}", testManifest(model.PluginSDKHostPoweroff))
	if err != nil {
		t.Fatal(err)
	}
	published, err := svc.Publish(ctx, actor, plugin.ID, rev.ID)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.CreateGrant(ctx, actor, model.PluginGrant{
	...
	next, err := svc.SaveDraft(ctx, actor, plugin.ID, "function main(){return {v:2}}", testManifest(model.PluginSDKHostPoweroff))
	if err != nil {
		t.Fatal(err)
	}
	newer, err := svc.Publish(ctx, actor, plugin.ID, next.ID)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.CreateGrant(ctx, actor, model.PluginGrant{
	...
	if _, err := svc.EnqueueManualRun(ctx, actor, plugin.ID, newer.ID, json.RawMessage(`{}`), json.RawMessage(`{}`), "live-1", model.PluginRunModeLive); err == nil {
...
func TestSimulateManageActionDoesNotCallHost(t *testing.T) {
	...
	ctx := context.Background()
	actor := adminActor(user)
	svc.SetCallerResolver(func(context.Context, model.PluginRun) (application.Principal, error) { return actor, nil })
	plugin, err := svc.CreatePlugin(ctx, actor, "sim", "")
	if err != nil {
		t.Fatal(err)
	}
	rev, err := svc.SaveDraft(ctx, actor, plugin.ID, "function main(){return {ok:true}}", testManifest(model.PluginSDKHostPoweroff))
	if err != nil {
		t.Fatal(err)
	}
	published, err := svc.Publish(ctx, actor, plugin.ID, rev.ID)
	...
	run, err := svc.EnqueueManualRun(ctx, actor, plugin.ID, published.ID, json.RawMessage(`{}`), json.RawMessage(`{}`), "sim-1", model.PluginRunModeSimulate)
	if err != nil {
		t.Fatal(err)
	}
	leased, err := db.LeasePluginRun(ctx, "worker-test", time.Now().Add(time.Minute), svc.Settings(ctx).RecoveryGeneration)
	if err != nil {
		t.Fatal(err)
	}
	host := &recordingHost{}
	gateway := NewGateway(db, host)
	resp, err := gateway.Invoke(ctx, svc, pluginrpc.SDKRequest{
		WorkerID: "worker-test", RunUUID: leased.UUID, LeaseGeneration: leased.LeaseGeneration,
		Capability: model.PluginSDKHostPoweroff, ActionKey: "power-1",
	...
	actions, err := db.ListPluginRunActions(ctx, run.ID)
...
func TestManualIdempotencyConflictOnDifferentPayload(t *testing.T) {
	...
	ctx := context.Background()
	actor := adminActor(user)
	plugin, err := svc.CreatePlugin(ctx, actor, "idem", "")
	if err != nil {
		t.Fatal(err)
	}
	rev, err := svc.SaveDraft(ctx, actor, plugin.ID, "function main(){return {ok:true}}", testManifest())
	if err != nil {
		t.Fatal(err)
	}
	published, err := svc.Publish(ctx, actor, plugin.ID, rev.ID)
	...
	first, err := svc.EnqueueManualRun(ctx, actor, plugin.ID, published.ID, json.RawMessage(`{}`), json.RawMessage(`{}`), "same-key", model.PluginRunModeSimulate)
	if err != nil {
		t.Fatal(err)
	}
	second, err := svc.EnqueueManualRun(ctx, actor, plugin.ID, published.ID, json.RawMessage(`{}`), json.RawMessage(`{}`), "same-key", model.PluginRunModeSimulate)
	if err != nil || second.ID != first.ID {
		t.Fatalf("identical replay must reuse run: %#v err=%v", second, err)
	}
	other, err := svc.SaveDraft(ctx, actor, plugin.ID, "function main(){return {other:true}}", testManifest())
	if err != nil {
		t.Fatal(err)
	}
	otherPub, err := svc.Publish(ctx, actor, plugin.ID, other.ID)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.EnqueueManualRun(ctx, actor, plugin.ID, otherPub.ID, json.RawMessage(`{}`), json.RawMessage(`{}`), "same-key", model.PluginRunModeSimulate); err == nil {
...
func (h *recordingHost) RestartService(context.Context, application.Principal, model.PluginRun, model.PluginRunAction, int64, string) (map[string]any, error) {
	h.restartCalls++
	return map[string]any{"operation_id": "real"}, nil
}
func (h *recordingHost) HostPower(context.Context, application.Principal, model.PluginRun, model.PluginRunAction, int64, string, string) (map[string]any, error) {
	h.powerCalls++
	return map[string]any{"operation_id": "real"}, nil
}
func (h *recordingHost) SendNotification(context.Context, application.Principal, model.PluginRun, model.PluginRunAction, int64, string, string) (map[string]any, error) {
	h.notifyCalls++
	return map[string]any{"operation_id": "real"}, nil
}
