...
func ParseTriggerSpec(raw []byte) (model.PluginTriggerSpec, error) {
	spec := model.PluginTriggerSpec{}
...
func NextSlots(spec model.PluginTriggerSpec, from time.Time, count int) ([]time.Time, error) {
	if count <= 0 {
		count = 5
	}
...
func DSTGap(spec model.PluginTriggerSpec, candidate time.Time) bool {
	loc, err := time.LoadLocation(spec.Timezone)
	if err != nil {
		return false
	}
