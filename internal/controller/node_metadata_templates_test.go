...
func createOrderTemplate(t *testing.T, h http.Handler, token, name string, regions []string) map[string]any {
	t.Helper()
	return request(t, h, http.MethodPost, "/api/v1/ui/node-order-templates", token, map[string]any{
		"name": name,
...
func TestGlobalRenameInheritanceAndClearPlanOverride(t *testing.T) {
	...
	createdB := request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans", token, map[string]any{
	...
	rename := request(t, h, http.MethodPatch, "/api/v1/ui/assignable-nodes/proxy_path/"+itoa(pathID)+"/metadata", token, map[string]any{
	...
	orderingB := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planBID)+"/ordering", token, nil, http.StatusOK)
	overrideSaved := request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planBID)+"/node-presentation/versions", token, map[string]any{
	...
	orderingA := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planAID)+"/ordering", token, nil, http.StatusOK)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planBID)+"/node-presentation/versions", token, map[string]any{
		"base_revision_id":      int64(orderingB["base_revision_id"].(float64)),
		"expected_lock_version": int64(orderingB["lock_version"].(float64)),
	...
	request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planBID)+"/revisions/"+itoa(overrideRevisionID)+"/restore", token, map[string]any{
		"expected_lock_version": int64(orderingB["lock_version"].(float64)),
	}, http.StatusOK)
	restoredB := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planBID)+"/ordering", token, nil, http.StatusOK)
...
func TestTemplateSnapshotPreserveManualAndRebuild(t *testing.T) {
	...
	request(t, h, http.MethodPost, "/api/v1/ui/node-order-templates", token, map[string]any{
	...
	initialApply := request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/ordering/apply-template", token, map[string]any{
	...
	request(t, h, http.MethodPatch, "/api/v1/ui/node-order-templates/"+itoa(templateID), token, map[string]any{
	...
	before := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/ordering", token, nil, http.StatusOK)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/ordering/apply-template", token, map[string]any{
	...
	preserved := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/ordering", token, nil, http.StatusOK)
	...
	rebuilt := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/ordering", token, nil, http.StatusOK)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/revisions/"+itoa(r1RevisionID)+"/restore", token, map[string]any{
		"expected_lock_version": int64(rebuilt["lock_version"].(float64)),
	}, http.StatusOK)
	restored := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/ordering", token, nil, http.StatusOK)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/node-order-templates/"+itoa(templateID)+"/archive", token, map[string]any{"expected_revision": 2}, http.StatusOK)
	archivedPlan := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/ordering", token, nil, http.StatusOK)
...
func TestPlanOrderingIsolation(t *testing.T) {
	...
	saveManual := func(planID int64, order []string) map[string]any {
		...
		return request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/ordering", token, nil, http.StatusOK)
	}
	...
	unchangedB := request(t, h, http.MethodGet, "/api/v1/ui/subscription-plans/"+itoa(planBID)+"/ordering", token, nil, http.StatusOK)
...
func TestGlobalRenameInvalidatesInheritedSubscriptionETag(t *testing.T) {
	...
	planID := ids["plan"]
	pathID := ids["p2"]
	createdUser := request(t, h, http.MethodPost, "/api/v1/ui/users", token, map[string]any{"username": "alias-user", "password": "long-user-password", "role": "viewer", "status": "active"}, http.StatusCreated)["user"].(map[string]any)
	userID := int64(createdUser["id"].(float64))
	subscriptionToken := createdUser["subscription_token"].(string)
	assignment := request(t, h, http.MethodPost, "/api/v1/ui/users/plan-assignment/apply", token, map[string]any{"user_ids": []int64{userID}, "plan_id": planID, "deploy": false}, http.StatusOK)
	...
	request(t, h, http.MethodPatch, "/api/v1/ui/assignable-nodes/proxy_path/"+itoa(pathID)+"/metadata", token, map[string]any{
