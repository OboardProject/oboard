...
For TLS inbounds (AnyTLS, HY2, VLESS WebSocket), pass the server, protocol or kind, port, and an operator-owned dns_domain. Those kinds default to dns_sync_enabled=true and certificate_mode=auto, so dns_credential_id is required. A single tenant DNS credential or bootstrap default_dns_credential_id is filled automatically. Otherwise oboard_task fails before ready with code missing_dns_credential and available_credentials [{id, name, provider}]. Changing dns_domain deletes the previous DNS records, writes the new ones, follows SNI unless a custom SNI was already set, immediately binds a ready covering certificate when one exists, and otherwise issues it on the next deploy. Do not wait for a ready certificate, do not create an external-mode placeholder, and do not send the operator to the panel to pre-issue the certificate. Controller matches or issues the managed certificate during deployment; issuance takes time and a later deploy picks it up once ready.
