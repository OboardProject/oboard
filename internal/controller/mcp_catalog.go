...
A Controller database write is not proof that every node has applied the change. RestrictedSSH additionally requires matching local SSH authentication verification for the current plan; an old successful task or active credential row is not proof. users_pending_reason=ssh_authentication_unverified requires updating Agent and reapplying the server configuration, not merely retrying runtime-user delivery. Authentication verification covers local password admission or expected policy rejection, not public reachability or downstream forwarding. ` + "`servers.list`" + ` / ` + "`servers.get`" + ` expose ` + "`authorization_confirmed`" + ` and ` + "`users_confirmed`" + `. Do not tell the operator that a user can already connect, or that a revocation has landed, until those fields are true on the affected servers, or ` + "`users_fallback=apply_core_config`" + ` and the matching core-config Workflow has succeeded. ` + "`servers.delivery.retry`" + ` only re-pushes the signed authorization lease and runtime-user lanes; it does not queue ` + "`apply_deployment`" + `.
...
Subscription client templates are configuration shells only. Use ` + "`subscription_templates.list/get/validate/preview`" + ` to read them and ` + "`subscription_templates.update/reset`" + ` to save or restore builtins. Do not put protocol field conversion, user passwords, or SSH host keys into a template. Saving a template does not queue Agent tasks. Public formats are mihomo, stash, surfboard, surge, surge-mac, loon, egern, shadowrocket, qx, sing-box, v2ray, and v2ray-uri. Omitted ` + "`format`" + ` is ` + "`mihomo`" + `. ` + "`format=auto`" + ` is User-Agent detection, not a renderer. ` + "`format=clash`" + `, ` + "`format=clash-meta`" + `, standalone ` + "`format=mieru`" + `, and ` + "`format=sing-box-mieru`" + ` are rejected; the Mieru protocol still appears in Mihomo, Shadowrocket, and Surge Mac Mihomo Bridge.

For TLS inbounds (AnyTLS, HY2, VLESS WebSocket), pass the server, protocol or kind, port, and a certificate-covering SNI (` + "`certificate_domain`" + `, or inherited from ` + "`dns_domain`" + `). ` + "`certificate_mode=auto`" + ` is the default. ` + "`dns_sync_enabled`" + ` defaults to false: SNI does not need to resolve to this host; when sync is off, subscription Host is the server public IP and TLS still completes from SNI plus the certificate. Pass ` + "`dns_domain`" + ` or set ` + "`dns_sync_enabled=true`" + ` only when the operator wants Controller to maintain DNS records. ` + "`dns_credential_id`" + ` is required only then; a single tenant credential or bootstrap ` + "`default_dns_credential_id`" + ` is filled automatically, otherwise ` + "`oboard_task`" + ` fails before ready with code ` + "`missing_dns_credential`" + ` and ` + "`available_credentials`" + ` [{id, name, provider}]. Fast Path treats a bare ` + "`certificate_domain`" + ` as SNI-only, and treats a ` + "`dns_domain`" + ` (parameter or named in the goal) as a record-sync request. Changing ` + "`dns_domain`" + ` deletes the previous DNS records, writes the new ones, follows SNI unless a custom SNI was already set, immediately binds a ready covering certificate when one exists, and otherwise issues it on the next deploy. Do not wait for a ready certificate, do not create an external-mode placeholder, and do not send the operator to the panel to pre-issue the certificate. Controller matches or issues the managed certificate during deployment; issuance takes time and a later deploy picks it up once ready.

Fast Path intent ` + "`server.manage`" + ` maps “重试授权 / 授权没下去” to ` + "`servers.delivery.retry`" + ` (no ` + "`apply_deployment`" + `) and “续费 / 延长到期” with ` + "`days`" + ` to ` + "`servers.extend_expiry`" + `. Do not invent a REST path for either.

User credentials are two rotate operations, not a revoke-to-empty action. “更换订阅地址 / 轮换订阅” maps to ` + "`subscriptions.rotate`" + `. “更换节点密码” maps to ` + "`users.credentials.rotate`" + `. Fast Path intent ` + "`user.credentials.manage`" + ` asks which one when the goal only says “用户凭证”. Do not create device-specific subscriptions, and do not revoke a subscription token to empty.

Scripts are a restricted ` + "`oboard-js-v1`" + ` environment, not Node.js and not remote shell. Use the stable script tools (` + "`scripts.list/get/create/update`" + `, ` + "`scripts.validate/simulate/run`" + `, ` + "`scripts.runs.get/cancel`" + `, ` + "`script_triggers.*`" + `). Do not invent a tool per script. Do not use ` + "`server_exec_shell`" + ` as a substitute. MCP cannot create grants, enable the runtime, or authorize host power; tell the operator to use the panel. Validate does not execute source. Simulate creates no real notifications or Agent tasks. An offline server usually cannot be powered off through its own Agent.
...
Fast Path intent ` + "`servers.runtime.refresh`" + ` maps “刷新全部节点配置 / 清理 Agent 配置” to ` + "`servers.runtime.refresh`" + ` (` + "`confirm: true`" + `). It force-rebuilds Agent runtime config, restarts the kernel, and re-pushes authorization and runtime users. Do not substitute ` + "`deployments.apply`" + ` or loop ` + "`servers.delivery.retry`" + `.`
