...
Subscription client templates are configuration shells only. Use ` + "`subscription_templates.list/get/validate/preview`" + ` to read them and ` + "`subscription_templates.update/reset`" + ` to save or restore builtins. Do not put protocol field conversion, user passwords, or SSH host keys into a template. Saving a template does not queue Agent tasks. Public formats are mihomo, stash, surfboard, surge, surge-mac, loon, egern, shadowrocket, qx, sing-box, sing-box-mieru, v2ray, and v2ray-uri. Omitted ` + "`format`" + ` is ` + "`mihomo`" + `. ` + "`format=auto`" + ` is User-Agent detection, not a renderer. ` + "`format=clash`" + `, ` + "`format=clash-meta`" + `, and standalone ` + "`format=mieru`" + ` are rejected; the Mieru protocol still appears in Mihomo, Shadowrocket, sing-box-mieru, and Surge Mac Mihomo Bridge.

For TLS inbounds (AnyTLS, HY2, VLESS WebSocket), pass the server, protocol or kind, port, and dns_domain. Set dns_sync_enabled when DNS records should be written; dns_credential_id is then required. A single tenant DNS credential or bootstrap default_dns_credential_id is filled automatically. Otherwise oboard_task fails before ready with code missing_dns_credential and available_credentials [{id, name, provider}]. Those kinds default to certificate_mode=auto. Do not wait for a ready certificate, do not create an external-mode placeholder, and do not send the operator to the panel to pre-issue the certificate. Controller matches or issues the managed certificate during deployment; issuance takes time and a later deploy picks it up once ready.
