...
Controller update success means the new Controller binary is available. It does not wait for Agent reconnect or fleet version sync and must not queue apply_deployment. Read ` + "`agent_updates.status`" + ` or ` + "`oboard://agent-updates`" + ` for rolling Agent progress. ` + "`agents.update_all`" + ` starts a bounded operator roll and keeps filling slots until enrolled Agents are current; never loop ` + "`servers.update_agent`" + ` across the inventory.`
