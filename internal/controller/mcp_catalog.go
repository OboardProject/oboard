...
Plugins are a restricted ` + "`oboard-js-v1`" + ` environment, not Node.js and not remote shell. Use the stable plugin tools (` + "`plugins.list/get/create/update`" + `, ` + "`plugins.validate/simulate/run`" + `, ` + "`plugins.runs.get/cancel`" + `, ` + "`plugin_triggers.*`" + `). Do not invent a tool per plugin. Do not use ` + "`server_exec_shell`" + ` as a substitute. MCP cannot create grants, enable the runtime, or authorize host power; tell the operator to use the panel. Validate does not execute source. Simulate creates no real notifications or Agent tasks. An offline server usually cannot be powered off through its own Agent.
