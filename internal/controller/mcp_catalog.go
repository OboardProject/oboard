...
Subscription client templates are configuration shells only. Use ` + "`subscription_templates.list/get/validate/preview`" + ` to read them and ` + "`subscription_templates.update/reset`" + ` to save or restore builtins. Do not put protocol field conversion, user passwords, or SSH host keys into a template. Saving a template does not queue Agent tasks. Public formats are mihomo, stash, surfboard, surge, surge-mac, loon, egern, shadowrocket, qx, sing-box, sing-box-mieru, v2ray, and v2ray-uri. ` + "`format=auto`" + ` is User-Agent detection, not a renderer. ` + "`format=clash`" + `, ` + "`format=clash-meta`" + `, and standalone ` + "`format=mieru`" + ` are rejected; the Mieru protocol still appears in Mihomo, Shadowrocket, sing-box-mieru, and Surge Mac Mihomo Bridge.
