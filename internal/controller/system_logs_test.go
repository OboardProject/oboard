...
func TestSystemLogsAdminAPI(t *testing.T) {
	...
	response := request(t, h, http.MethodGet, "/api/v1/ui/system-logs?lines=20&q=unique", token, nil, http.StatusOK)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/settings", token, map[string]any{"controller_log_max_mb": 2, "controller_log_backups": 1}, http.StatusOK)
	config := logs.Config()
	if config.MaxBytes != 2<<20 || config.Backups != 1 {
		t.Fatalf("runtime log config = %#v", config)
	}

	request(t, h, http.MethodPost, "/api/v1/ui/system-logs", token, map[string]any{"action": "rotate"}, http.StatusOK)
	download := httptest.NewRecorder()
	downloadReq := httptest.NewRequest(http.MethodGet, "/api/v1/ui/system-logs/download", nil)
	...
	request(t, h, http.MethodDelete, "/api/v1/ui/system-logs", token, nil, http.StatusOK)
...
func TestAgentLogSettingsAndControlTasks(t *testing.T) {
	...
	configTask := request(t, h, http.MethodPost, "/api/v1/ui/servers/"+itoa(server.ID)+"/agent-config", token, map[string]any{
	...
	request(t, h, http.MethodPost, "/api/v1/ui/servers/"+itoa(server.ID)+"/agent-config", token, map[string]any{"log_max_mb": 0}, http.StatusBadRequest)
	controlTask := request(t, h, http.MethodPost, "/api/v1/ui/servers/"+itoa(server.ID)+"/logs/control", token, map[string]any{"action": "rotate", "services": "core"}, http.StatusAccepted)
	if controlTask["task"].(map[string]any)["type"] != "manage_logs" {
		t.Fatalf("unexpected log control task: %#v", controlTask)
	}
	request(t, h, http.MethodPost, "/api/v1/ui/servers/"+itoa(server.ID)+"/logs/control", token, map[string]any{"action": "remove", "services": "all"}, http.StatusBadRequest)
}
...
func TestSystemLogsRejectNonAdmin(t *testing.T) {
	...
	viewerLogin := request(t, h, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "viewer", "password": "long-user-password"}, http.StatusOK)
	request(t, h, http.MethodGet, "/api/v1/ui/system-logs", viewerLogin["token"].(string), nil, http.StatusForbidden)
}
