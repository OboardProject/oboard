...
func TestTimeCheckSettingsAndServerModeChangeQueueImmediately(t *testing.T) {
	...
	settings := request(t, h, http.MethodGet, "/api/v1/settings", token, nil, http.StatusOK)["settings"].(map[string]any)
	...
	created := request(t, h, http.MethodPost, "/api/v1/servers", token, map[string]any{
	...
	response := request(t, h, http.MethodPatch, "/api/v1/servers/"+itoa(serverID), token, server, http.StatusOK)
...
func TestDailyTimeCheckStoresResultAndNotifiesAdminsOnce(t *testing.T) {
	...
	viewer := request(t, h, http.MethodPost, "/api/v1/users", adminToken, map[string]any{"username": "viewer", "password": "long-viewer-password", "role": "viewer", "status": "active"}, http.StatusCreated)["user"].(map[string]any)
	...
	request(t, h, http.MethodPost, "/api/v1/notification-channels", adminToken, map[string]any{"name": "admin-clock", "type": "telegram", "enabled": true, "events": notificationServerOffline, "config_json": `{"bot_token":"admin","chat_id":"1"}`}, http.StatusCreated)
	request(t, h, http.MethodPost, "/api/v1/notification-channels", viewerToken, map[string]any{"name": "viewer-other", "type": "bark", "enabled": true, "events": notificationAdminAnnouncement, "config_json": `{"device_key":"viewer"}`}, http.StatusCreated)
