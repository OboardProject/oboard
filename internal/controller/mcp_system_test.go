...
func TestSettingsCapabilities(t *testing.T) {
	...
	updateInput, _ := json.Marshal(map[string]any{"changes": map[string]any{"resource_download_cn_controller": true, "resource_download_source": "github", "audit_enabled": false, "traffic_timezone": "Asia/Tokyo", "subscription_relay_url": "https://subscriptions.example.com", "subscription_controller_direct_enabled": true, "mcp_enabled": true}})
	...
	if settings[resourceDownloadCNControllerSetting] != "true" || settings[resourceDownloadSourceSetting] != "github" || settings["audit_enabled"] != "false" || settings["traffic_timezone"] != "Asia/Tokyo" || settings["subscription_relay_url"] != "https://subscriptions.example.com" || settings[settingSubscriptionControllerDirectEnabled] != "true" || settings[settingMCPEnabled] != "true" {
	...
	for _, input := range []string{`{"changes":{"resource_download_cn_controller":"true"}}`, `{"changes":{"resource_download_cn_controller":null}}`, `{"changes":{"resource_download_source":"latest"}}`, `{"changes":{"resource_download_source":true}}`, `{"changes":{"resource_download_source":null}}`} {
