...
func TestBinaryOnlyControllerReleaseAssets(t *testing.T) {
	...
	required := map[string][]string{
		"scripts/build-release.sh":                                 {"create_tar_archive \"$stage\" \"$archive\" bin/oboard-controller bin/oboard-controller-updater bin/oboard-ai-worker bin/oboard-plugin-worker web downloads", "${arch}_install.tar.gz", "package_subscription_relay", "relay-release/oboard-subscription-relay-", "subscription-relay-sha256s.txt", "install-subscription-relay.sh", "deploy/systemd", "deploy/openrc"},
		...
		"scripts/install.sh":                                       {"OBOARD_UPDATE_CHANNEL", "oboard-controller-updater", "install_component controller", "prepare_controller_updater_runtime", "uninstall_controller", "OBOARD_PURGE_DATA", "resolve_purge_data", "drain_piped_script", "enable-plugins", "want_plugin_runtime"},
