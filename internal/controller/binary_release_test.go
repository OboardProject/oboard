...
func TestBinaryOnlyControllerReleaseAssets(t *testing.T) {
	...
	required := map[string][]string{
		"scripts/build-release.sh":                                 {"create_tar_archive \"$stage\" \"$archive\" bin/oboard-controller bin/oboard-controller-updater bin/oboard-ai-worker web downloads", "${arch}_install.tar.gz", "package_subscription_relay", "relay-release/oboard-subscription-relay-", "subscription-relay-sha256s.txt", "install-subscription-relay.sh", "deploy/systemd", "deploy/openrc"},
