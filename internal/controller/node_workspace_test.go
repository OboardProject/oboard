...
func TestNodeWorkspaceOwnershipImportOutputAndShare(t *testing.T) {
	...
	first := request(t, handler, http.MethodPost, "/api/v2/ui/users", adminToken, map[string]any{"username": "first", "password": "first-user-password", "role": "none", "status": "active"}, http.StatusCreated)["user"].(map[string]any)
	second := request(t, handler, http.MethodPost, "/api/v2/ui/users", adminToken, map[string]any{"username": "second", "password": "second-user-password", "role": "viewer", "status": "active"}, http.StatusCreated)["user"].(map[string]any)
	firstID := int64(first["id"].(float64))
	secondID := int64(second["id"].(float64))
	firstToken := request(t, handler, http.MethodPost, "/api/v2/ui/auth/login", "", map[string]any{"username": "first", "password": "first-user-password"}, http.StatusOK)["token"].(string)

	workspace := request(t, handler, http.MethodGet, "/api/v2/ui/node-workspace", firstToken, nil, http.StatusOK)
	...
	request(t, handler, http.MethodGet, "/api/v2/ui/node-workspace?user_id="+itoa(secondID), firstToken, nil, http.StatusForbidden)
	managed := request(t, handler, http.MethodGet, "/api/v2/ui/node-workspace?user_id="+itoa(firstID), adminToken, nil, http.StatusOK)
	if managed["subject"].(map[string]any)["username"] != "first" {
		t.Fatalf("admin perspective subject = %#v", managed["subject"])
	}

	created := request(t, handler, http.MethodPost, "/api/v2/ui/node-groups", firstToken, map[string]any{
	...
	library := request(t, handler, http.MethodGet, "/api/v2/ui/node-library", firstToken, nil, http.StatusOK)["nodes"].([]any)
	if len(library) != 2 {
		t.Fatalf("private node library count = %d, want 2", len(library))
	}
	nodeID := library[0].(map[string]any)["id"].(string)
	shared := request(t, handler, http.MethodPost, "/api/v2/ui/node-library/share", firstToken, map[string]any{"node_id": nodeID}, http.StatusOK)
	if shared["url"] == "" {
		t.Fatal("node share URL is empty")
	}

	output := request(t, handler, http.MethodPost, "/api/v2/ui/subscription-outputs", firstToken, map[string]any{"name": "私人组合", "group_ids": []int64{groupID}}, http.StatusCreated)["subscription_output"].(map[string]any)
	outputID := int64(output["id"].(float64))
	preview := request(t, handler, http.MethodPost, "/api/v2/ui/subscription-outputs/"+itoa(outputID)+"/preview", firstToken, map[string]any{"format": "sing-box"}, http.StatusOK)["preview"].(map[string]any)
	...
	duplicate := request(t, handler, http.MethodPost, "/api/v2/ui/node-groups", firstToken, map[string]any{
	...
	request(t, handler, http.MethodPatch, "/api/v2/ui/subscription-outputs/"+itoa(outputID), firstToken, map[string]any{"name": "私人组合", "group_ids": []int64{groupID, duplicateID}}, http.StatusOK)
	previewResponse := request(t, handler, http.MethodPost, "/api/v2/ui/subscription-outputs/"+itoa(outputID)+"/preview", firstToken, map[string]any{"format": "sing-box"}, http.StatusOK)
	if previewResponse["deduplicated_count"] != float64(1) {
		t.Fatalf("deduplicated_count = %#v, want 1", previewResponse["deduplicated_count"])
	}

	request(t, handler, http.MethodPost, "/api/v2/ui/node-groups", firstToken, map[string]any{"name": "blocked", "kind": "remote", "url": "https://127.0.0.1/sub"}, http.StatusBadRequest)
}
...
func TestNodeWorkspaceInvalidProfileIsRejected(t *testing.T) {
	...
	server := newTestServer(db, "test-secret", "")
	handler := server.Handler()
	created := request(t, handler, http.MethodPost, "/api/v2/ui/auth/bootstrap", "", map[string]any{"username": "admin", "password": "very-secure-password"}, http.StatusCreated)
...
func TestNodeWorkspaceProfileParticipatesInSubscriptionETag(t *testing.T) {
	...
	adminToken := request(t, handler, http.MethodPost, "/api/v2/ui/auth/login", "", map[string]any{"username": "admin", "password": "very-secure-password"}, http.StatusOK)["token"].(string)
	workspace := request(t, handler, http.MethodGet, "/api/v2/ui/node-workspace", adminToken, nil, http.StatusOK)
	...
	second := request(t, handler, http.MethodPost, "/api/v2/ui/subscription-outputs", adminToken, map[string]any{"name": "同内容组合", "group_ids": []int64{groupID}}, http.StatusCreated)["subscription_output"].(map[string]any)
