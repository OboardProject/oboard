...
func TestAIProviderTestUsesDraftAndStoredCredentials(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			response := request(t, handler, http.MethodPost, "/api/v1/ai/provider-test", token, testCase.requestBody, http.StatusOK)
...
func TestAIProviderTestFailureSurfacesDetailAndRedactsRawResponse(t *testing.T) {
	...
	response := request(t, handler, http.MethodPost, "/api/v1/ai/provider-test", token, map[string]any{"base_url": "http://127.0.0.1:11434/v1", "model": "manual", "api_key": "secret-key"}, http.StatusOK)
	if err := <-workerErr; err != nil {
		t.Fatal(err)
	}
	data := response["data"].(map[string]any)
...
func TestAIProviderTestRejectsInvalidInput(t *testing.T) {
	...
	request(t, handler, http.MethodPost, "/api/v1/ui/auth/bootstrap", "", map[string]any{"username": "admin", "password": "very-secure-password"}, http.StatusCreated)
	token := request(t, handler, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "admin", "password": "very-secure-password"}, http.StatusOK)["token"].(string)
	request(t, handler, http.MethodPost, "/api/v1/ai/provider-test", token, map[string]any{"base_url": "https://api.example.com/v1", "api_key": "secret"}, http.StatusBadRequest)
	request(t, handler, http.MethodPost, "/api/v1/ai/provider-test", token, map[string]any{"base_url": "https://api.example.com/v1?key=value", "api_key": "secret", "model": "m"}, http.StatusBadRequest)
	request(t, handler, http.MethodPost, "/api/v1/ai/provider-test", token, map[string]any{"base_url": "https://api.example.com/v1", "model": "m"}, http.StatusBadRequest)
}
