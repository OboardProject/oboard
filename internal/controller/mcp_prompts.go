...
Intents: `+"`dns_policy.manage`"+`, `+"`dns_record.manage`"+`, `+"`port_forward.manage`"+`, `+"`tunnel.manage`"+`, `+"`host_ops.manage`"+` (MTU detection), and inbound TLS create via `+"`inbound.create`"+`. For AnyTLS/HY2/VLESS WebSocket, pass an operator-owned dns_domain; DNS sync is on by default. Controller issues the managed certificate during deployment. Changing the domain replaces DNS records and rematches or issues the covering certificate. Do not wait for a ready certificate or send the operator to the panel. Follow needs_input or choose_candidate with the continuation_id and commit only the returned prepared_id after confirmation. Follow the Workflow until terminal.
