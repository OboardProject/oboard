...
func (s *Server) apiV2AIProvider(w http.ResponseWriter, r *http.Request) {
	id := strings.Trim(strings.TrimPrefix(r.URL.Path, "/api/v2/ai/providers/"), "/")
	item, err := s.store.GetAIProvider(r.Context(), id)
	if err != nil {
		v2HandleError(w, r, err)
		return
	}
	if r.Method == http.MethodDelete {
		if err := s.store.DeleteAIProvider(r.Context(), id); err != nil {
	...
	if request.APIKey != nil && strings.TrimSpace(*request.APIKey) != "" {
		item.CredentialEncrypted, err = security.EncryptSecret(s.sessionSecret, "ai-provider-credential:"+id, strings.TrimSpace(*request.APIKey))
		if err != nil {
			v2HandleError(w, r, err)
			return
		}
	...
	item.HasCredential, item.CredentialEncrypted = item.CredentialEncrypted != "", ""
	v2Write(w, r, http.StatusOK, item, nil)
}
