...
func (s *Server) prepareSettingsRecipe(ctx context.Context, principal application.Principal, input mcpTaskInput) (*mcpPreparedRecipe, error) {
	...
	for _, key := range []string{"stealth_transport", "audit_enabled", "subscription_audit_enabled", "connection_audit_enabled", "audit_action", "traffic_timezone", "subscription_age_policy", "subscription_always_use_domain_host", "subscription_custom_path_mode", "server_default_mtu_mode", "server_default_bbr_enabled", "server_default_tcp_tuning_enabled", "server_default_time_correction_mode", "server_monitoring_retention_days", "connection_audit_retention_days", "time_check_ntp_servers", "trusted_proxy_cidrs", "controller_log_max_mb", "controller_log_backups", "resource_download_source", "resource_download_cn_controller", "agent_auto_update_enabled", "subscription_relay_auto_update_enabled", "update_window_enabled", "update_window_start_hour", "update_window_end_hour", "agent_update_max_concurrency", "managed_update_startup_quiet_seconds", "registration_enabled"} {
		if value, ok := input.Params[key]; ok {
			changes[key] = value
		}
	}
