...
func TestSubscriptionAgeAPIOptionalRequiredAndHeaderModes(t *testing.T) {
	...
	created := request(t, h, http.MethodPost, "/api/v1/ui/users", adminToken, map[string]any{"username": "age-user", "password": "long-user-password", "role": "viewer", "status": "active"}, http.StatusCreated)
	user := created["user"].(map[string]any)
	userID := int64(user["id"].(float64))
	subscriptionToken := user["subscription_token"].(string)
	userLogin := request(t, h, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "age-user", "password": "long-user-password"}, http.StatusOK)
	...
	configured := request(t, h, http.MethodPatch, "/api/v1/ui/users/"+itoa(userID)+"/subscription-age", adminToken, map[string]any{"enabled": true, "public_key": publicKey}, http.StatusOK)
	...
	request(t, h, http.MethodPatch, "/api/v1/ui/users/"+itoa(userID)+"/subscription-age", adminToken, map[string]any{"enabled": true, "public_key": identity.String()}, http.StatusBadRequest)
	request(t, h, http.MethodPatch, "/api/v1/ui/users/"+itoa(userID)+"/subscription-age", userToken, map[string]any{"enabled": true, "public_key": publicKey}, http.StatusForbidden)
	...
	request(t, h, http.MethodPatch, "/api/v1/ui/users/"+itoa(userID)+"/subscription-age", adminToken, map[string]any{"enabled": false, "public_key": publicKey}, http.StatusOK)
	...
	optionalSelf := request(t, h, http.MethodPatch, "/api/v1/ui/me/subscription-age", userToken, map[string]any{"enabled": true, "public_key": publicKey}, http.StatusOK)
	...
	settings := request(t, h, http.MethodPost, "/api/v1/ui/settings", adminToken, map[string]any{"subscription_age_policy": "required"}, http.StatusOK)
	...
	missing := request(t, h, http.MethodPost, "/api/v1/ui/users", adminToken, map[string]any{"username": "missing-key", "password": "long-user-password", "role": "viewer", "status": "active"}, http.StatusCreated)
	...
	missingLogin := request(t, h, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "missing-key", "password": "long-user-password"}, http.StatusOK)
	missingSession := missingLogin["token"].(string)
	selfIdentity, err := age.GenerateX25519Identity()
	if err != nil {
		t.Fatal(err)
	}
	selfUpdate := request(t, h, http.MethodPatch, "/api/v1/ui/me/subscription-age", missingSession, map[string]any{"enabled": false, "public_key": selfIdentity.Recipient().String()}, http.StatusOK)
