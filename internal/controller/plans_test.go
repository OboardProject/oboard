...
func TestSubscriptionPlansAndAssignmentAPI(t *testing.T) {
	...
	path := request(t, h, http.MethodPost, "/api/v1/ui/proxy-paths", token, map[string]any{"inbound_id": inboundID, "enabled": true}, http.StatusCreated)["proxy_path"].(map[string]any)
	...
	catalog := request(t, h, http.MethodGet, "/api/v1/ui/assignable-nodes", token, nil, http.StatusOK)
	...
	preview := request(t, h, http.MethodPost, "/api/v1/ui/users/plan-assignment/preview", token, map[string]any{"user_ids": []int64{userID}, "plan_id": planID}, http.StatusOK)["preview"].(map[string]any)
	if len(preview["nodes_added"].([]any)) != 1 {
		t.Fatalf("preview = %#v", preview)
	}
	applied := request(t, h, http.MethodPost, "/api/v1/ui/users/plan-assignment/apply", token, map[string]any{"user_ids": []int64{userID}, "plan_id": planID, "deploy": false}, http.StatusOK)
	...
	detail = request(t, h, http.MethodGet, "/api/v1/ui/assignable-nodes/proxy_path/"+itoa(pathID), token, nil, http.StatusOK)
	...
	preview = request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/nodes/preview", token, map[string]any{"op": "replace", "nodes": []map[string]any{}}, http.StatusOK)["preview"].(map[string]any)
	if len(preview["nodes_removed"].([]any)) != 1 {
		t.Fatalf("replace preview = %#v", preview)
	}
	request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/nodes/sync", token, map[string]any{"op": "replace", "nodes": []map[string]any{}}, http.StatusOK)
	published := request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/publish", token, map[string]any{}, http.StatusOK)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/nodes/sync", token, map[string]any{
	...
	request(t, h, http.MethodPost, "/api/v1/ui/user-node-exceptions", token, map[string]any{
...
func driveAccessChange(t *testing.T, srv *Server, token string, changeID int64) map[string]any {
	...
	for i := 0; i < 10; i++ {
		...
		change = request(t, srv.Handler(), http.MethodGet, "/api/v1/ui/access-changes/"+itoa(changeID), token, nil, http.StatusOK)["access_change"].(map[string]any)
...
func TestPlanModeAccessChangeFlow(t *testing.T) {
	...
	applied := request(t, h, http.MethodPost, "/api/v1/ui/users/plan-assignment/apply", token, map[string]any{"user_ids": []int64{userID}, "plan_id": planID, "deploy": true}, http.StatusOK)
	...
	change := request(t, h, http.MethodGet, "/api/v1/ui/access-changes/"+itoa(changeID), token, nil, http.StatusOK)["access_change"].(map[string]any)
	...
	path2Inbound := request(t, h, http.MethodPost, "/api/v1/ui/inbounds", token, map[string]any{"server_id": serverID, "name": "vless2", "protocol": "vless", "listen_ip": "0.0.0.0", "port": 8443, "config_json": `{}`, "enabled": true}, http.StatusCreated)["inbound"].(map[string]any)
	path2 := request(t, h, http.MethodPost, "/api/v1/ui/proxy-paths", token, map[string]any{"inbound_id": int64(path2Inbound["id"].(float64)), "enabled": true}, http.StatusCreated)["proxy_path"].(map[string]any)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/subscription-plans/"+itoa(planID)+"/changes/apply", token, map[string]any{
	...
	ex := request(t, h, http.MethodPost, "/api/v1/ui/user-node-exceptions", token, map[string]any{
	...
	deny := request(t, h, http.MethodPatch, "/api/v1/ui/user-node-exceptions/"+itoa(exceptionID), token, map[string]any{
