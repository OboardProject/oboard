...
func TestNotificationChannelCRUDAndTestValidation(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/ui/notification-channels", bytes.NewReader(body))
	...
	created := request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", token, map[string]any{
	...
	testReq := httptest.NewRequest(http.MethodPost, "/api/v1/ui/notification-channels/"+itoa(id)+"/test", bytes.NewReader([]byte("{}")))
	...
	draftReq := httptest.NewRequest(http.MethodPost, "/api/v1/ui/notification-channels/test", bytes.NewReader(draftBody))
...
func TestTelegramBotSecretIsStoredOutsideNotificationChannels(t *testing.T) {
	...
	updated := request(t, h, http.MethodPut, "/api/v1/ui/telegram-bot", token, map[string]any{"enabled": true, "bot_token": "123:abc"}, http.StatusOK)
...
func TestNotificationChannelRoleScopeOwnershipAndTemplates(t *testing.T) {
	...
	createdUser := request(t, h, http.MethodPost, "/api/v1/ui/users", adminToken, map[string]any{"username": "viewer", "password": "long-viewer-password", "role": "viewer", "status": "active"}, http.StatusCreated)
	...
	page := request(t, h, http.MethodGet, "/api/v1/ui/page-data?page=notifications", viewerToken, nil, http.StatusOK)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", viewerToken, map[string]any{
	...
	createdViewerChannel := request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", viewerToken, map[string]any{
	...
	request(t, h, http.MethodGet, "/api/v1/ui/notification-channels/"+itoa(viewerChannelID), adminToken, nil, http.StatusNotFound)
	adminList := request(t, h, http.MethodGet, "/api/v1/ui/notification-channels", adminToken, nil, http.StatusOK)
	if len(adminList["notification_channels"].([]any)) != 0 {
		t.Fatalf("admin could list another user's channels: %#v", adminList)
	}
	createdAdminChannel := request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", adminToken, map[string]any{
	...
	request(t, h, http.MethodGet, "/api/v1/ui/notification-channels/"+itoa(int64(adminChannel["id"].(float64))), viewerToken, nil, http.StatusNotFound)
}
...
func TestNotificationDispatchScopeTemplatesAndDedupe(t *testing.T) {
	...
	adminToken := adminLogin["token"].(string)
	adminID := int64(adminLogin["user"].(map[string]any)["id"].(float64))
	createdUser := request(t, h, http.MethodPost, "/api/v1/ui/users", adminToken, map[string]any{"username": "viewer", "nickname": "小王", "password": "long-viewer-password", "role": "viewer", "status": "active"}, http.StatusCreated)
	viewerID := int64(createdUser["user"].(map[string]any)["id"].(float64))
	viewerLogin := request(t, h, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "viewer", "password": "long-viewer-password"}, http.StatusOK)
	viewerToken := viewerLogin["token"].(string)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", adminToken, map[string]any{
...
func TestConnectionAuditRiskNotificationTargetsUserAndAdmin(t *testing.T) {
	...
	createdUser := request(t, h, http.MethodPost, "/api/v1/ui/users", adminToken, map[string]any{"username": "viewer", "nickname": "小王", "password": "long-viewer-password", "role": "viewer", "status": "active"}, http.StatusCreated)
	viewerID := int64(createdUser["user"].(map[string]any)["id"].(float64))
	viewerToken := request(t, h, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "viewer", "password": "long-viewer-password"}, http.StatusOK)["token"].(string)
	request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", viewerToken, map[string]any{"name": "viewer-risk", "type": "bark", "enabled": true, "events": notificationUserRisk, "config_json": `{"device_key":"viewer"}`}, http.StatusCreated)
	request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", adminToken, map[string]any{"name": "admin-risk", "type": "bark", "enabled": true, "events": notificationUserRisk, "user_ids": []int64{viewerID}, "config_json": `{"device_key":"admin"}`}, http.StatusCreated)
...
func TestTaskTimeoutAndAdminAnnouncementQueue(t *testing.T) {
	...
	viewerLogin := request(t, h, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "viewer", "password": "long-viewer-password"}, http.StatusOK)
	viewerToken := viewerLogin["token"].(string)
	request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", adminToken, map[string]any{"name": "admin", "type": "bark", "enabled": true, "events": notificationTaskTimeout, "config_json": `{"device_key":"admin"}`}, http.StatusCreated)
	request(t, h, http.MethodPost, "/api/v1/ui/notification-channels", viewerToken, map[string]any{"name": "viewer", "type": "bark", "enabled": true, "events": notificationAdminAnnouncement, "config_json": `{"device_key":"viewer"}`}, http.StatusCreated)
	...
	request(t, h, http.MethodPost, "/api/v1/ui/notification-announcements", viewerToken, map[string]any{"title": "no", "body": "no", "user_ids": []int64{viewerID}}, http.StatusForbidden)
	...
	announcement := request(t, h, http.MethodPost, "/api/v1/ui/notification-announcements", adminToken, map[string]any{"title": "维护", "body": "今晚维护", "user_ids": []int64{viewerID}}, http.StatusAccepted)
	...
	waitNotificationCount(t, srv, &sentMu, &sent, 2)
	history := request(t, h, http.MethodGet, "/api/v1/ui/notification-announcements", adminToken, nil, http.StatusOK)
...
func TestNotificationTestChannelAndRawLog(t *testing.T) {
	...
	login := request(t, h, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "admin", "password": "very-secure-password"}, http.StatusOK)
	token := login["token"].(string)
	request(t, h, http.MethodPost, "/api/v1/ui/users", token, map[string]any{"username": "viewer", "password": "long-viewer-password", "role": "viewer", "status": "active"}, http.StatusCreated)
	viewerLogin := request(t, h, http.MethodPost, "/api/v1/ui/auth/login", "", map[string]any{"username": "viewer", "password": "long-viewer-password"}, http.StatusOK)
	viewerToken := viewerLogin["token"].(string)
	request(t, h, http.MethodGet, "/api/v1/ui/notification-channels/raw-log?lines=100", viewerToken, nil, http.StatusForbidden)
	...
	request(t, h, http.MethodPost, fmt.Sprintf("/api/v1/ui/notification-channels/%d/test", id), token, map[string]any{}, http.StatusOK)

	if _, err := logs.Write([]byte("unrelated log line\nnotification[test] channel=调试 title=\"OBoard 测试通知\" body=\"原始消息正文\"\n")); err != nil {
		t.Fatal(err)
	}
	response := request(t, h, http.MethodGet, "/api/v1/ui/notification-channels/raw-log?lines=100", token, nil, http.StatusOK)
	...
	invalid := request(t, h, http.MethodGet, "/api/v1/ui/notification-channels/raw-log?lines=9999", token, nil, http.StatusBadRequest)
	if invalid == nil {
