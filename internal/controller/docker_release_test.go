...
func TestDockerInstallAndReleaseAssets(t *testing.T) {
	...
	checks := map[string][]string{
		...
		"scripts/install-docker.sh":           {"VERSION_VALUE", "OBOARD_TAG", "docker compose", "docker-cli", "docker-compose-v2", "docker_compose_ready", "wait_for_health", "dev|development|nightly", "generate_admin_password", "configure_bootstrap_admin", "设置超级管理员", "自动加入“管理员组”", "不能在面板中删除", "Controller 和 Agent 相互独立，也可以安装在同一台服务器上", "OBOARD_PORT 必须是 1 到 65535", "OBOARD_BASE_PATH", "更新当前渠道", "2787"},
