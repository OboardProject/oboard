...
func TestDockerInstallAndReleaseAssets(t *testing.T) {
	...
	checks := map[string][]string{
		...
		"scripts/verify-release.sh":           {"Testing Controller", "Testing Agent", "Testing oboard-sb", "Building Web UI", "Building current-platform binaries"},
		".github/workflows/ci.yml":            {"verify-release.sh", "OboardProject/oboard-agent", "go-version: '1.25.11'", "node-version: '22'"},
		".github/workflows/dev-build.yml":     {"packages: write", "linux/amd64,linux/arm64", ":dev", "build-release.sh"},
		".github/workflows/prerelease.yml":    {":prerelease", "build-push-action", "v${{ steps.meta.outputs.version }}"},
		".github/workflows/release.yml":       {":latest", "build-push-action", "v${{ inputs.version }}"},
	}
