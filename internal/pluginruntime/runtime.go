...
type SDKFunc func(capability string, arguments json.RawMessage, actionKey string) (pluginrpc.SDKResponse, error)
type LogFunc func(level, message string, fields json.RawMessage)

func Execute(source string, params json.RawMessage, env map[string]string, limits pluginrpc.RunLimits, invoke SDKFunc, log LogFunc) (json.RawMessage, error) {
	if err := plugin.ValidateSource(source); err != nil {
		return nil, err
	}
	vm := goja.New()
	...
	interrupt := time.AfterFunc(time.Until(deadline), func() { vm.Interrupt("plugin timed out") })
	...
	for _, name := range []string{
		...
		"management.query", "management.preview", "management.apply", "network.request", "config.get",
	} {
		capability := name
