...
func TestIntermediateDirectBranchRoutesAtItsSourceServer(t *testing.T) {
	...
	for _, name := range []string{"A｜C", "A｜B", "A｜C｜直出"} {
		if !names[name] {
...
func TestTransparentPortForwardMovesUserProtocolToProcessingServer(t *testing.T) {
  ...
  "tls":{
	...
	if forward.Backend != model.ForwardBackendRealm {
		t.Fatalf("transparent entry forward backend = %s, want realm", forward.Backend)
	}
	...
	if processing == nil || processing["type"] != "vless" || intFromAny(processing["listen_port"]) == 557 {
		t.Fatalf("processing server missing cloned VLESS inbound: %s", configB)
	}
	if intFromAny(processing["listen_port"]) != forward.TargetPort {
		t.Fatalf("processing inbound %#v does not listen on the forward target port %d", processing, forward.TargetPort)
	}
