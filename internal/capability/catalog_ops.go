...
func opsDescriptors(positiveID map[string]any, stringValue, boolValue map[string]any, nullableString, nullableInteger func() map[string]any) []Descriptor {
	agentTask := closedObject(map[string]any{
		...
		"operations": arrayOf(closedObject(map[string]any{"id": stringValue, "total": map[string]any{"type": "integer"}, "pending": map[string]any{"type": "integer"}, "running": map[string]any{"type": "integer"}, "succeeded": map[string]any{"type": "integer"}, "failed": map[string]any{"type": "integer"}, "unknown": map[string]any{"type": "integer"}})),
	})
	syncProblem := closedObject(map[string]any{"code": stringValue, "category": stringValue, "retry_policy": stringValue, "message": stringValue, "resources": arrayOf(closedObject(map[string]any{"type": stringValue, "id": stringValue}))})
	syncState := closedObject(map[string]any{"server_id": positiveID, "desired_revision": map[string]any{"type": "integer"}, "state": stringValue, "config_version": map[string]any{"type": "integer"}, "task_id": map[string]any{"type": "integer"}, "retry_count": map[string]any{"type": "integer"}, "error": stringValue, "problems": map[string]any{"type": "array", "maxItems": 16, "items": syncProblem}, "changed_at": stringValue, "updated_at": stringValue, "next_retry_at": stringValue, "agent_reachable": boolValue})
	operation := closedObject(map[string]any{"id": stringValue, "kind": stringValue, "source": stringValue, "actor_principal": stringValue, "actor_user_id": nullableInteger(), "created_at": stringValue, "attempts": arrayOf(closedObject(map[string]any{"target_type": stringValue, "target_id": stringValue, "task_id": nullableInteger(), "attempt": map[string]any{"type": "integer"}, "config_version": nullableInteger(), "actual_revision": nullableInteger(), "execution_kind": stringValue, "state": stringValue})), "targets": arrayOf(closedObject(map[string]any{"target_type": stringValue, "target_id": stringValue, "state": stringValue, "cause_code": stringValue, "desired_revision": nullableInteger(), "updated_at": stringValue}))})
	reads := []Descriptor{
		{Name: "task_operations.list", Description: "分页读取完整授权的操作及全部目标结果，包括未入队失败", InputSchema: schemaObject(map[string]any{"before_time": stringValue, "before_id": stringValue, "limit": map[string]any{"type": "integer", "minimum": 1, "maximum": 100}}), OutputSchema: schemaObject(map[string]any{"operations": arrayOf(operation)}, "operations"), RequiredScopes: []string{"tasks:read"}, ResourceTypes: []string{"server"}, ResourceEvaluator: "server_ids", ReadOnly: true, Idempotent: true, DataClassification: DataInternal, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, ResolveResourceRefs: noRefs},
		{Name: "task_operations.get", Description: "读取完整授权的操作及全部目标结果", InputSchema: schemaObject(map[string]any{"id": stringValue}, "id"), OutputSchema: rawSchema(operation), RequiredScopes: []string{"tasks:read"}, ResourceTypes: []string{"server"}, ResourceEvaluator: "server_ids", ReadOnly: true, Idempotent: true, DataClassification: DataInternal, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, ResolveResourceRefs: noRefs},
		{Name: "configuration_sync.get", Description: "读取服务器的配置同步状态与结构化问题", InputSchema: schemaObject(map[string]any{"server_id": positiveID}, "server_id"), OutputSchema: rawSchema(syncState), RequiredScopes: []string{"tasks:read"}, ResourceTypes: []string{"server"}, ResourceEvaluator: "server_ids", ReadOnly: true, Idempotent: true, DataClassification: DataInternal, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, ResolveResourceRefs: noRefs},
	...
	writes := []struct {
		...
		{"servers.runtime.refresh", "强制重建全部已接入 Agent 的运行配置并重启内核；授权凭证、运行时用户、延迟探测计划与流量策略都会以新版本号重新下发。连接会短暂中断。需要 confirm=true。不要用普通 deployments.apply 代替", schemaObject(map[string]any{"confirm": map[string]any{"type": "boolean", "const": true}}, "confirm"), schemaObject(map[string]any{"config_version": map[string]any{"type": "integer"}, "traffic_policy_revision": map[string]any{"type": "integer"}, "queued_tasks": map[string]any{"type": "integer"}, "queued_servers": map[string]any{"type": "integer"}, "failed_immediate": map[string]any{"type": "integer"}, "delivery_retried": map[string]any{"type": "integer"}, "reissued_servers": map[string]any{"type": "integer"}, "skipped_unenrolled": map[string]any{"type": "integer"}, "server_ids": map[string]any{"type": "array", "items": positiveID}}, "config_version"), 3, true},
