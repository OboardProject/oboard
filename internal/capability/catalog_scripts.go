...
func scriptDescriptors(positiveID, stringValue, boolValue map[string]any, nullableString, nullableInteger func() map[string]any) []Descriptor {
	...
	return []Descriptor{
		...
		{Name: "script_runtime.status", Description: "读取脚本 Worker、沙箱诊断和调度暂停状态", InputSchema: schemaObject(nil), OutputSchema: schemaObject(map[string]any{"status": closedObject(map[string]any{})}, "status"), RequiredScopes: []string{"scripts:read"}, ReadOnly: true, Idempotent: true, DataClassification: DataInternal, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, RBACPermission: "scripts.read", ResolveResourceRefs: noRefs},
		{Name: "script_runtime.settings.update", Description: "管理员启停脚本运行环境或调整系统限额。MCP 不可调用", InputSchema: schemaObject(map[string]any{"enabled": boolValue, "host_actions_enabled": boolValue, "scheduler_paused": boolValue, "max_concurrency": map[string]any{"type": "integer", "minimum": 1, "maximum": 8}, "max_timeout_seconds": map[string]any{"type": "integer", "minimum": 5, "maximum": 300}}), OutputSchema: schemaObject(map[string]any{"status": closedObject(map[string]any{})}, "status"), RequiredScopes: []string{"scripts:settings"}, RiskClass: 4, ApprovalPolicy: "required", Idempotent: true, DataClassification: DataInternal, MCPEnabled: false, Executable: true, MinimumAccess: mcpauth.AccessOperate, RBACPermission: "scripts.settings", AdminOnly: true, ResolveResourceRefs: noRefs},
