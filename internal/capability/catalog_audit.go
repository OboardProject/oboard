...
func auditDescriptors(positiveID map[string]any, stringValue, boolValue map[string]any, nullableString, nullableInteger func() map[string]any) []Descriptor {
	...
	reads := []Descriptor{
		...
		{Name: "audit.logs.list", Description: "列出审计操作日志", InputSchema: schemaObject(map[string]any{"limit": map[string]any{"type": "integer", "minimum": 1, "maximum": 500}}), OutputSchema: schemaObject(map[string]any{"logs": arrayOf(closedObject(map[string]any{"id": map[string]any{"type": "integer"}, "actor": stringValue, "action": stringValue, "target": stringValue, "detail": stringValue, "ip": stringValue, "created_at": stringValue})), "count": map[string]any{"type": "integer"}}, "logs"), RequiredScopes: []string{"audit:read"}, ReadOnly: true, Idempotent: true, DataClassification: DataSensitive, SensitiveFields: []string{"user_identity"}, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, RBACPermission: "admin.settings", ResolveResourceRefs: noRefs},
