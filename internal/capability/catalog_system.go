...
func systemDescriptors(positiveID map[string]any, stringValue, boolValue map[string]any, nullableString, nullableInteger func() map[string]any) []Descriptor {
	...
	apiPrincipal := closedObject(map[string]any{
		...
		"scopes": stringArray(1, 128), "resource_filter": map[string]any{"type": "object"}, "allowed_cidrs": stringArray(0, 64),
		"rate_limit_per_minute": map[string]any{"type": "integer"},
		"max_concurrency":       map[string]any{"type": "integer"}, "expires_at": nullableString(),
		"last_used_at": nullableString(), "created_at": stringValue, "updated_at": stringValue,
	})
	apiPrincipalFieldProperties := map[string]any{
		"name":                  map[string]any{"type": "string", "minLength": 1, "maxLength": 80},
		"enabled":               boolValue,
		"scopes":                stringArray(1, 128),
		"resource_filter":       map[string]any{"type": "object"},
		"allowed_cidrs":         stringArray(0, 64),
		"rate_limit_per_minute": map[string]any{"type": "integer", "minimum": 1, "maximum": 10000},
		"max_concurrency":       map[string]any{"type": "integer", "minimum": 1, "maximum": 64},
		"expires_at":            nullableString(),
	}
	apiPrincipalCreateFields := closedObject(apiPrincipalFieldProperties, "name", "scopes")
	apiPrincipalUpdateFields := closedObject(apiPrincipalFieldProperties)
	apiToken := closedObject(map[string]any{
		"id": stringValue, "principal_id": stringValue, "prefix": stringValue,
		"expires_at": stringValue, "revoked_at": nullableString(), "last_used_at": nullableString(), "created_at": stringValue,
	})
	...
	controllerUpdate := closedObject(map[string]any{
		"channel": stringValue, "current": controllerBuild, "available": controllerBuild,
		...
		"operation": closedObject(map[string]any{
			...
			"backup": closedObject(map[string]any{
				"total_pages": map[string]any{"type": "integer"}, "remaining_pages": map[string]any{"type": "integer"},
	...
	descriptors := []Descriptor{
		...
		adminRead("api_tokens.list", "列出指定服务账号的令牌元数据（不含令牌明文）", schemaObject(map[string]any{"principal_id": stringValue}, "principal_id"), rawSchema(arrayOf(apiToken))),
		...
		adminWrite("settings.update", "修改主控全局设置；stealth_transport 必须单独保存，校验端口后立即生效，已有安全进程 Agent 时禁止改址或关闭；WebSSH 密码确认开关仅允许管理员在 Web 面板重新验证身份后修改；subscription_relay_url 仅能设为已接入中继的公开地址或空字符串；subscription_always_use_domain_host 控制订阅 Host 是否始终使用解析域名", schemaObject(map[string]any{"changes": closedObject(map[string]any{
			...
			"server_default_mtu_mode":                stringValue, "server_default_bbr_enabled": boolValue, "server_default_tcp_tuning_enabled": boolValue,
			...
			"resource_download_cn_controller":        boolValue,
			...
			"managed_update_startup_quiet_seconds": map[string]any{"type": "integer", "minimum": 0, "maximum": 300},
		...
		adminWrite("subscription_relays.activate", "将当前在线（最近 2 分钟内有心跳）的受管订阅中继设为订阅公开入口", schemaObject(map[string]any{"relay_id": positiveID}, "relay_id"), schemaObject(map[string]any{"relay_id": positiveID, "active": boolValue}, "relay_id", "active"), 2, false),
		...
		adminWrite("backups.create", "在后台创建一次手动主控备份", schemaObject(map[string]any{"upload_remote": boolValue}), schemaObject(map[string]any{"backup": closedObject(map[string]any{"id": stringValue, "name": stringValue, "origin": stringValue, "status": stringValue})}, "backup"), 2, false),
		adminWrite("approval_policies.set", "新增或更新一条自动化审批策略", schemaObject(map[string]any{"principal_id": stringValue, "capability": stringValue, "mode": map[string]any{"type": "string", "enum": []string{"denied", "required", "automatic"}}, "allow_risk4": boolValue, "resource_filter": map[string]any{"type": "object"}, "expires_at": nullableString()}, "principal_id", "capability", "mode"), schemaObject(map[string]any{"approval_policy": approvalPolicy}, "approval_policy"), 2, false),
		...
		adminWrite("api_principals.create", "创建受范围、资源边界和限流约束的服务账号", schemaObject(map[string]any{"service_account": apiPrincipalCreateFields}, "service_account"), schemaObject(map[string]any{"api_principal": apiPrincipal}, "api_principal"), 3, false),
		adminWrite("api_principals.update", "修改服务账号的状态、范围、资源边界或限流", schemaObject(map[string]any{"principal_id": stringValue, "changes": apiPrincipalUpdateFields}, "principal_id", "changes"), schemaObject(map[string]any{"api_principal": apiPrincipal}, "api_principal"), 3, false),
		...
		adminWrite("api_tokens.issue", "为服务账号签发一个仅显示一次的访问令牌", schemaObject(map[string]any{"principal_id": stringValue, "expires_at": nullableString()}, "principal_id"), schemaObject(map[string]any{"api_token": stringValue, "token_info": apiToken}, "api_token", "token_info"), 3, false),
		adminWrite("api_tokens.revoke", "撤销指定服务账号的访问令牌", schemaObject(map[string]any{"principal_id": stringValue, "token_id": stringValue, "confirm": map[string]any{"type": "boolean", "const": true}}, "principal_id", "token_id", "confirm"), schemaObject(map[string]any{"revoked": boolValue, "token_info": apiToken}, "revoked", "token_info"), 3, true),
	...
	for index := range descriptors {
		...
		case "api_tokens.issue":
			descriptors[index].DataClassification = DataSensitive
			descriptors[index].SensitiveOutput = []string{"api_token"}
		}
	}
	return descriptors
}
