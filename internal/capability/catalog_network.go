...
func networkDescriptors(positiveID map[string]any, stringValue, boolValue map[string]any, nullableString, nullableInteger func() map[string]any) []Descriptor {
	...
	snellProfile := closedObject(map[string]any{
		...
		"reuse": boolValue, "tcp_fast_open": boolValue, "remark": stringValue, "builtin": boolValue, "enabled": boolValue,
	...
	subscriptionTemplate := closedObject(map[string]any{
		...
		"builtin_updated": boolValue, "markers": stringArray(1, 8),
		"updated_by": map[string]any{"type": "integer"}, "updated_at": stringValue,
	})
	reads := []Descriptor{
		...
		{Name: "subscription_templates.list", Description: "列出全部订阅客户端模板（系统默认或管理员覆盖）。模板只描述配置外壳，不包含协议字段转换。", InputSchema: schemaObject(nil), OutputSchema: rawSchema(arrayOf(subscriptionTemplate)), RequiredScopes: []string{"subscription_templates:read"}, ResourceTypes: []string{"server"}, ResourceEvaluator: "server_ids", ReadOnly: true, Idempotent: true, DataClassification: DataInternal, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, RBACPermission: "admin.settings", ResolveResourceRefs: noRefs},
		{Name: "subscription_templates.get", Description: "读取指定客户端的订阅模板", InputSchema: schemaObject(map[string]any{"format": subscriptionTemplateFormat}, "format"), OutputSchema: rawSchema(subscriptionTemplate), RequiredScopes: []string{"subscription_templates:read"}, ResourceTypes: []string{"server"}, ResourceEvaluator: "server_ids", ReadOnly: true, Idempotent: true, DataClassification: DataInternal, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, RBACPermission: "admin.settings", ResolveResourceRefs: noRefs},
		{Name: "subscription_templates.validate", Description: "校验订阅客户端模板标记与协议预览，不保存", InputSchema: schemaObject(map[string]any{"format": subscriptionTemplateFormat, "content": map[string]any{"type": "string", "minLength": 1}}, "format", "content"), OutputSchema: schemaObject(map[string]any{"valid": boolValue}, "valid"), RequiredScopes: []string{"subscription_templates:read"}, ResourceTypes: []string{"server"}, ResourceEvaluator: "server_ids", ReadOnly: true, Idempotent: true, DataClassification: DataInternal, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, RBACPermission: "admin.settings", ResolveResourceRefs: noRefs},
		{Name: "subscription_templates.preview", Description: "用合成协议节点预览订阅客户端模板输出，不含真实用户密码", InputSchema: schemaObject(map[string]any{"format": subscriptionTemplateFormat, "content": map[string]any{"type": "string", "minLength": 1}}, "format", "content"), OutputSchema: schemaObject(map[string]any{"format": subscriptionTemplateFormat, "content": stringValue}, "format", "content"), RequiredScopes: []string{"subscription_templates:read"}, ResourceTypes: []string{"server"}, ResourceEvaluator: "server_ids", ReadOnly: true, Idempotent: true, DataClassification: DataInternal, MCPEnabled: true, MinimumAccess: mcpauth.AccessRead, RBACPermission: "admin.settings", ResolveResourceRefs: noRefs},
	...
	writes := []struct {
		...
		{"subscription_templates.update", "保存订阅客户端模板覆盖。只改配置外壳，不触发 Agent 任务。", schemaObject(map[string]any{"format": map[string]any{"type": "string"}, "content": map[string]any{"type": "string", "minLength": 1}, "expected_revision": map[string]any{"type": "integer", "minimum": 0}}, "format", "content", "expected_revision"), schemaObject(map[string]any{"subscription_template": map[string]any{"type": "object"}}, "subscription_template"), 2, false, true},
		{"subscription_templates.reset", "删除订阅客户端模板覆盖并恢复系统默认模板。不触发 Agent 任务。", schemaObject(map[string]any{"format": map[string]any{"type": "string"}}, "format"), schemaObject(map[string]any{"subscription_template": map[string]any{"type": "object"}}, "subscription_template"), 2, true, true},
...
func networkScopeFor(name string) string {
	...
	case name == "subscription_templates.update" || name == "subscription_templates.reset":
		return "subscription_templates:write"
