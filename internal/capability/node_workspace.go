...
func nodeWorkspaceDescriptors(positiveID, stringValue, boolValue map[string]any) []Descriptor {
	...
	writes := []Descriptor{
		...
		{Name: "node_groups.update", Description: "更新节点组名称、远程订阅 URL 或手动节点链接", InputSchema: schemaObject(map[string]any{"user_id": positiveID, "group_id": positiveID, "name": map[string]any{"type": "string", "minLength": 1, "maxLength": 80}, "url": stringValue, "content": stringValue}, "user_id", "group_id"), SensitiveFields: []string{"url", "content"}, SensitiveInput: []string{"url", "content"}},
