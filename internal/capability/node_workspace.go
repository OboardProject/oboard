...
func nodeWorkspaceDescriptors(positiveID, stringValue, boolValue map[string]any) []Descriptor {
	...
	writes := []Descriptor{
		...
		{Name: "subscription_outputs.save", Description: "创建或更新有序组合订阅", InputSchema: schemaObject(map[string]any{"user_id": positiveID, "output_id": map[string]any{"type": "integer", "minimum": 0}, "name": map[string]any{"type": "string", "minLength": 1, "maxLength": 80}, "group_ids": groupIDs, "enabled": boolValue}, "user_id", "name", "group_ids")},
