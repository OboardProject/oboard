...
func latencyChartOutputSchema() json.RawMessage {
	...
	stats := map[string]any{"measurement_revision_count": integer, "key": text, "kind": text, "task_id": integer, "task_name": text, "mode": text, "province": text, "carrier": text, "sample_count": integer, "success_count": integer, "report_count": integer, "available_count": integer, "peak_latency_at": nullableTime, "peak_loss_at": nullableTime}
	...
	coverage := closedObject(map[string]any{"source": text, "target_ids": arrayOf(integer), "legacy_measurement_revision": boolean, "measurement_revision_count": integer, "retention_clipped": boolean, "has_samples": boolean, "legacy_curve_reports": boolean}, "source", "retention_clipped", "has_samples", "legacy_curve_reports")
