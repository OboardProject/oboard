...
import { readMonitorChart } from './monitor-chart-retry'
...
export function useServerMonitorQuery<T>(client: Client, path: string, enabled: boolean) {
  ...
  const [state, setState] = useState<{
    ...
    retrying: boolean
    error: unknown
  }>({ path, client, response: cached?.response ?? null, loading: !cached?.fresh, retrying: false, error: null })

  useCoalescedReadRequest<T>(`${clientKey}:${path}:${revision}`, signal => cache
    ? readMonitorChart(signal, () => cache.read<T>(path, signal, sharedSignal => client.request(path, { signal: sharedSignal })), () => {
      setState(previous => previous.path === path && previous.client === client ? { ...previous, retrying: true } : previous)
    })
    : client.request(path, { signal }), {
    onStart: () => setState(previous => ({ path, client, response: previous.path === path && previous.client === client ? previous.response : cached?.response ?? null, loading: !cache?.get<T>(path)?.fresh, retrying: false, error: null })),
    onSuccess: response => setState({ path, client, response, loading: false, retrying: false, error: null }),
    onError: error => setState(previous => ({ ...previous, loading: false, retrying: false, error })),
  }, { enabled })
  ...
  return {
    ...
    retrying: state.path === path && state.client === client && state.retrying,
