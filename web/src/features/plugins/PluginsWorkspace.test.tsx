...
describe('PluginsWorkspace', () => {
  ...
  it('loads plugin details and explicitly opens development tools for validation', async () => {
    ...
    await flush()
    expect(document.querySelector('[role="status"]')?.textContent).toContain('校验通过')
    expect(notify).not.toHaveBeenCalled()
  })

  it('keeps the latest editor and blocks duplicate pending validations', async () => {
    ...
    const requestV2 = vi.fn(async (path: string) => {
      if (path === '/plugins') return { plugins: [{ id: 1, name: '插件 A', status: 'draft' }, { id: 2, name: '插件 B', status: 'draft' }] }
      if (path === '/plugin-runtime/status') return { status: { runtime_installed: true } }
      if (path === '/plugins/1' && ++readsA > 1) return old
      if (path === '/plugins/1') return { plugin: { id: 1, name: '插件 A' } }
      if (path === '/plugins/2') return { plugin: { id: 2, name: '插件 B' }, draft: { source: 'source-B', manifest: {} } }
      if (path === '/plugins/2/validate') return validation
      return {}
    })
    await act(async () => root.render(<PluginsWorkspace tab="plugins" data={{ session: { role: 'admin' } }} client={{ requestV2 }} notify={vi.fn()} onNavigate={vi.fn()} />))
    await flush()
    const clickText = async (text: string) => {
      const button = Array.from(document.body.querySelectorAll('button')).find(item => item.textContent === text)
      expect(button).toBeDefined()
      await act(async () => { button!.click() })
      await flush()
    }
    const develop = async (index: number) => {
      await act(async () => { container.querySelectorAll<HTMLButtonElement>('.plugin-library-row')[index].click() })
      await flush()
      await clickText('开发')
      await clickText('打开源码编辑器')
    }
    await develop(0)
    await develop(1)
    await act(async () => { finishOld({ draft: { source: 'source-A', manifest: {} } }); await old })
    expect(document.querySelector<HTMLTextAreaElement>('[role="dialog"] textarea')?.value).toBe('source-B')
    const validate = Array.from(document.body.querySelectorAll('button')).find(item => item.textContent === '校验')!
    act(() => { validate.click(); validate.click() })
    expect(requestV2.mock.calls.filter(([path]) => path === '/plugins/2/validate')).toHaveLength(1)
    await act(async () => { finishValidation({ valid: true }); await validation })
    expect(document.querySelector('[role="status"]')?.textContent).toContain('校验通过')
  })
