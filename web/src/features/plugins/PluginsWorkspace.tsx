...
export function PluginsWorkspace({ tab, data, client, notify, onNavigate }: PluginsWorkspaceProps) {
  ...
  const refreshWorkspace = async () => {
    ...
    try {
      ...
      notify(error.message || '加载插件失败', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const saveCurrentDraft = async () => {
    ...
    const next = await api.getPlugin(requestV2, selected.id)
    setDetail(next)
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">安装插件，审核权限，管理自动化与插件页面。</p>
        </div>
      ...
      <Tabs value={view} onValueChange={(value) => onNavigate(value === 'triggers' ? 'plugin-triggers' : value === 'runs' ? 'plugin-runs' : 'plugins')}>
        ...
        <div className="plugin-runtime-status">
          ...
          {runtimeLoading && !runtime ? (
            ...
            <p>未安装。默认安装主控时不带插件运行环境；在主控主机上执行安装命令后再启用执行。</p>
          )}
          {isAdmin && runtime && (
      ...
      <Dialog isOpen={editorOpen} onClose={() => setEditorOpen(false)} title={selected ? `编辑 ${selected.name}` : '编辑插件'} size="xl">
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="sm" onClick={async () => {
              try {
                ...
                setDetail(await api.getPlugin(requestV2, selected!.id))
              } catch (error: any) { notify(error.message, 'error') }
            }}>发布版本</Button>
          </div>
