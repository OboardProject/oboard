...
export function isIndeterminateMutationFailure(error: unknown): boolean {
  if (isUnknownMutationOutcome(error)) return true
...
export function markIndeterminateMutation<T>(error: T): T {
  if (!isIndeterminateMutationFailure(error) || !error || typeof error !== 'object' || !Object.isExtensible(error)) return error
  ...
  target.message = reason
    ? `${unknownMutationPrefix}：${reason}。请核对当前状态与操作记录，确认前不要重复提交`
    : `${unknownMutationPrefix}，请核对当前状态与操作记录，确认前不要重复提交`
  return error
}
