...
describe('configuration save UI contract', () => {
  ...
  it('shows save/sync state and exposes retry only through failed sync rows', () => {
    ...
    expect(syncStatus).toContain('onLocateInbound(issue.inboundID!)')
    expect(syncStatus).toContain('configuration-sync-popover')
    expect(syncStatus).toContain('正在同步的服务器')
    expect(style).toContain('.deploy-status-pill.has-popover:hover .configuration-sync-popover')
    expect(main).toContain('flowInstance.getNode(`entry-${inboundID}`)')
    expect(main).toContain('setProxyInboundFocus({ inboundID, requestID: Date.now() })')
    expect(main).toContain('已定位并选中入口')
    expect(main).toContain('className="proxy-focus-announcement" role="status" aria-live="polite"')
