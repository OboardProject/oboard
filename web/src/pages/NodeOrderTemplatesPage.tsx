...
export function NodeOrderTemplatesPage({ data, client, notify }: { data: any; client: AnyClient; notify?: (message: string, tone?: any) => void }) {
  ...
  const save = async () => {
    ...
    try {
      const result = editor === 'new'
        ? await client.request<{ template: Template }>('/node-order-templates', { method: 'POST', body: JSON.stringify(draft) })
        : await client.request<{ template: Template }>(`/node-order-templates/${editor.id}`, { method: 'PATCH', body: JSON.stringify({ ...draft, expected_revision: editor.revision }) })
      if (session.current === savingSession) {
        setEditor(JSON.stringify(currentDraft.current) === JSON.stringify(snapshot) ? null : result.template)
      }
      void load()
    } catch (e: any) { const message = e?.message || String(e); if (session.current === savingSession) setError(message.includes('409') ? '模板已被其他人修改，你的输入已保留。请关闭并重新打开查看最新内容。' : message) } finally { saving.current = false; setBusy(false) }
  }
  ...
  const runPreview = async () => {
    ...
    setBusy(true); setError('')
    try {
      const result = await client.request(`/node-order-templates/${editor.id}/preview`, { method: 'POST', body: JSON.stringify({ plan_id: previewPlanID, policy: draft.policy }) })
      if (session.current === previewSession && currentDraft.current === snapshot) setPreview(result)
    } catch (e: any) { if (session.current === previewSession) setError(e?.message || String(e)) } finally { setBusy(false) }
  }
  ...
  return (
    <div className="panel node-template-panel">
      <div className="panel-body">
        ...
        {loading && templates.length === 0 ? <p className="muted">正在加载排序模板…</p> : (
      ...
      <Dialog isOpen={editor !== null} onClose={() => { session.current++; setEditor(null) }} title={editor === 'new' ? '新建排序模板' : `编辑模板 · ${(editor as Template)?.name}`} size="xl">
        {error && <p className="danger-text" role="alert">{error}</p>}
        ...
        <div className="dialog-actions"><Button variant="ghost" onClick={() => { session.current++; setEditor(null) }}>关闭</Button><Button busy={busy} onClick={() => void save()}><Save size={15} /> 保存模板</Button></div>
      </Dialog>
    </div>
  )
}
