...
describe('server expiry helpers', () => {
  ...
  it('derives statuses from expiry date and auto renewal', () => {
    ...
    expect(serverExpiryStatusValue('2026-08-08T00:00:00Z', true, now)).toEqual({ label: '6 天后到期', tone: 'danger' })
    expect(serverExpiryStatusValue('2026-08-09T00:00:00Z', true, now)).toEqual({ label: '7 天后到期', tone: 'warning' })
    expect(serverExpiryStatusValue('2026-08-10T00:00:00Z', true, now)).toEqual({ label: '8 天后到期', tone: 'warning' })
    expect(serverExpiryStatusValue('2026-08-16T00:00:00Z', true, now)).toEqual({ label: '14 天后到期', tone: 'warning' })
    expect(serverExpiryStatusValue('2026-08-17T00:00:00Z', true, now)).toEqual({ label: '15 天后到期', tone: 'ok' })
