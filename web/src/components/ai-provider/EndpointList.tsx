...
export function EndpointList({ endpoints }: { endpoints: AIProviderEndpoint[] }) {
  return <div className="ai-endpoint-list">{endpoints.map(endpoint => <div className="ai-endpoint-row" key={endpoint.id}>
    ...
    <div className="ai-endpoint-capability"><CapabilityBadge capability={endpoint.capability} />{endpoint.capability ? <small>{capabilityOutputModeLabel(endpoint.capability)}</small> : null}</div>
  </div>)}</div>
}
