...
export function RemoteAccessSettings({ data, client, load, notify }: { data: any; client: { request: RequestFn }; load: () => Promise<void>; notify: (message: string, tone?: string) => void }) {
  ...
  const [mcp, setMcp] = useState(settingEnabled(data.settings?.mcp_enabled, false))
  ...
  useEffect(() => {
    ...
    setMcp(settingEnabled(data.settings?.mcp_enabled, false))
    setPasswordConfirmation(settingEnabled(data.settings?.remote_terminal_password_confirmation_enabled, true))
  }, [data.settings?.remote_terminal_enabled, data.settings?.mcp_enabled, data.settings?.remote_terminal_password_confirmation_enabled])
  ...
  return (
    <>
      ...
      >
        ...
        <SettingsSwitchRow
          label="MCP 远程控制"
          description="打开后，该用户的所有 MCP 连接都允许全部远程控制权限（主机操作、结构化执行、Shell、交互式终端）。仍需 Privileged Grant 授权具体服务器。"
          checked={mcp}
          onChange={checked => {
            setMcp(checked)
            void save('mcp', { mcp_enabled: checked }, checked ? 'MCP 远程控制已开启' : 'MCP 远程控制已关闭')
          }}
          disabled={Boolean(saving)}
          ariaLabel="启用 MCP 远程控制"
        />
        <SettingsSwitchRow
          label="WebSSH 密码确认"
        ...
        {loading ? <p className="muted remote-access-server-loading">正在读取服务器设置…</p> : rows.length === 0 ? <div className="remote-access-server-empty"><ServerIcon size={20} aria-hidden="true" /><p>暂无服务器</p></div> : <>
          <div className="remote-access-bulk-bar">
            ...
            <div>
              ...
              <button type="button" className="ghost" disabled={busy || selected.size === 0} onClick={() => void patchRows(selectedRows, { mcp_enabled: true }, '已批量开启 MCP')}>开启 MCP</button>
              <button type="button" className="ghost" disabled={busy || selected.size === 0} onClick={() => void patchRows(selectedRows, { mcp_enabled: false }, '已批量关闭 MCP')}>关闭 MCP</button>
            </div>
          </div>
          <div className="remote-access-server-list">
            ...
            {rows.map(row => <div className="remote-access-server-row" key={row.id}>
              ...
              <Switch checked={row.mcp} disabled={busy} onChange={checked => void patchRows([row], { mcp_enabled: checked }, `${row.name || `服务器 ${row.id}`} 已更新`)} ariaLabel={`${row.name || `服务器 ${row.id}`}MCP`} />
            </div>)}
          </div>
        </>}
      </div>
