...
it('shows anomalies as the default tab without eagerly loading secondary details', async () => {
  await act(async () => root.render(<ConnectivityDetails serverID={1} windowKey="1h" client={client} anomalyCount={2} anomalyPanel={<p>异常摘要内容</p>} />))
  expect(container.querySelector('[role="tab"][aria-selected="true"]')?.textContent).toBe('异常事件 2')
  expect(container.textContent).toContain('异常摘要内容')
  expect(pending).toHaveLength(0)
  await click('在线率与故障')
  expect(pending[0].path).toContain('view=sla')
  expect(container.textContent).not.toContain('异常摘要内容')
})

it('loads only the selected detail tab and cancels requests when tabs or windows change', async () => {
  await act(async () => root.render(<ConnectivityDetails key="1h" serverID={1} windowKey="1h" client={client} />))
  expect(pending[0].path).toContain('view=sla')
  expect(container.querySelector('[role="tab"][aria-selected="true"]')?.textContent).toBe('在线率与故障')
  ...
  expect(pending[2].path).toContain('window=24h')
  expect(container.querySelector('[role="tab"][aria-selected="true"]')?.textContent).toBe('在线率与故障')
})

it('replaces event pages without accumulating history and refreshes once on visibility restore', async () => {
  await act(async () => root.render(<ConnectivityDetails serverID={1} windowKey="1h" client={client} anomalyPanel={<p>异常摘要内容</p>} />))
  await click('诊断事件')
