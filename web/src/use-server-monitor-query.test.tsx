...
it('keeps existing chart data while recovering a transient 503, and cancels backoff on window switch', async () => {
  vi.useFakeTimers()
  try {
    await act(async () => root.render(<Monitor />))
    await act(async () => pending[0].resolve({ value: 'existing' }))
    await act(async () => current.refresh())
    await act(async () => pending[1].reject(Object.assign(new Error('request canceled'), { status: 503 })))
    expect(current.response).toEqual({ value: 'existing' })
    expect(current.loading).toBe(true)
    expect(current.retrying).toBe(true)
    expect(current.error).toBeNull()
    await act(async () => { await vi.advanceTimersByTimeAsync(6000) })
    expect(pending).toHaveLength(3)
    await act(async () => pending[2].resolve({ value: 'recovered' }))
    expect(current.response).toEqual({ value: 'recovered' })
    expect(current.retrying).toBe(false)
    await act(async () => current.refresh())
    await act(async () => pending[3].reject(Object.assign(new Error('request canceled'), { status: 503 })))
    await act(async () => root.render(<Monitor window="7d" />))
