...
import {
  ...
  isControllerUpdateRestartPhase,
  monotonicPercent,
...
describe('controller update pending toast', () => {
  ...
  it('shows the pending info toast during an update on TypeError', () => {
    expect(controllerUpdatePendingToast(true, new TypeError('Failed to fetch'), 'installing')).toEqual({ message: CONTROLLER_UPDATE_PENDING_MESSAGE, kind: 'info' })
    expect(controllerUpdatePendingToast(true, new TypeError('Failed to fetch'), 'restarting')).toEqual({ message: CONTROLLER_UPDATE_PENDING_MESSAGE, kind: 'info' })
  })

  it('shows the pending info toast during an update on network failures', () => {
    expect(controllerUpdatePendingToast(true, new Error('NetworkError when attempting to fetch resource'), 'installing')).toEqual({ message: CONTROLLER_UPDATE_PENDING_MESSAGE, kind: 'info' })
    expect(controllerUpdatePendingToast(true, 'Failed to fetch', 'restarting')).toEqual({ message: CONTROLLER_UPDATE_PENDING_MESSAGE, kind: 'info' })
  })
  ...
  it.each(['Bad Gateway', 'Service Unavailable', 'Gateway Timeout', '502 Bad Gateway'])('shows the pending info toast for gateway unavailability: %s', (message) => {
    expect(controllerUpdatePendingToast(true, new Error(message), 'installing')).toEqual({ message: CONTROLLER_UPDATE_PENDING_MESSAGE, kind: 'info' })
  })

  it('keeps the normal failure path outside an update', () => {
    expect(controllerUpdatePendingToast(false, new TypeError('Failed to fetch'), 'installing')).toBeNull()
    expect(controllerUpdatePendingToast(false, new Error('Bad Gateway'), 'restarting')).toBeNull()
  })

  it('recognizes gateway unavailability by HTTP status even when the message is generic', () => {
    const error = new Error('操作失败，请稍后重试')
    ;(error as any).status = 502
    expect(controllerUpdatePendingToast(true, error, 'installing')).toEqual({ message: CONTROLLER_UPDATE_PENDING_MESSAGE, kind: 'info' })
    for (const status of [502, 503, 504]) {
      const statusError = new Error('')
      ;(statusError as any).status = status
      expect(controllerUpdatePendingToast(true, statusError, 'restarting')).toEqual({ message: CONTROLLER_UPDATE_PENDING_MESSAGE, kind: 'info' })
    }
  })
  ...
  it('does not treat other HTTP statuses as expected disconnects', () => {
    for (const status of [401, 403, 500, 5024]) {
      ...
      expect(controllerUpdatePendingToast(true, error, 'installing')).toBeNull()
    }
  })

  it('does not mask real errors during an update', () => {
    expect(controllerUpdatePendingToast(true, new Error('forbidden'), 'installing')).toBeNull()
    expect(controllerUpdatePendingToast(true, new Error('subscription_age_policy must be optional or required'), 'installing')).toBeNull()
  })

  it('does not soften disconnects outside the installing/restarting phases', () => {
    for (const phase of ['checking', 'downloading', 'preflight', 'backing_up', 'ready', 'verifying', 'cancelling', undefined]) {
      expect(controllerUpdatePendingToast(true, new TypeError('Failed to fetch'), phase)).toBeNull()
    }
  })

  it('only classifies installing/restarting as restart phases', () => {
    expect(isControllerUpdateRestartPhase('installing')).toBe(true)
    expect(isControllerUpdateRestartPhase('restarting')).toBe(true)
    expect(isControllerUpdateRestartPhase('checking')).toBe(false)
    expect(isControllerUpdateRestartPhase('downloading')).toBe(false)
    expect(isControllerUpdateRestartPhase('verifying')).toBe(false)
    expect(isControllerUpdateRestartPhase(undefined)).toBe(false)
  })
